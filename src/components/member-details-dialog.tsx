...
export function MemberDetailsDialog({ details, onClose, onRevoke }: { details: UserDetails; onClose: () => void; onRevoke: () => Promise<void> }) {
  ...
  return <div className="admin-dialog-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget && !confirmRevoke) onClose(); }}>
    <section className="admin-dialog user-details-dialog" role="dialog" aria-modal="true" aria-labelledby="user-details-title">
      ...
      <div className="user-details-scroll">
        ...
        <div className="user-account-flags"><span>Role: <strong>{details.member.role}</strong></span><span>Status: <strong>{details.member.status}</strong></span><span>Email: <strong>{details.member.emailVerifiedAt ? "Verified" : "Unverified"}</strong></span></div>
