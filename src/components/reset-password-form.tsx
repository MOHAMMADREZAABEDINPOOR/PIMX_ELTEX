...
export function ResetPasswordForm() {
  ...
  return <div className="auth-page"><section className="auth-card"><Brand /><h1>Choose a new password.</h1><p>Use at least 12 characters with upper and lowercase letters, a number, and a symbol.</p><form className="auth-form" onSubmit={submit}><div className="field"><label htmlFor="reset-code">Recovery code</label><input id="reset-code" name="code" inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, "").slice(0, 6))} required aria-invalid={Boolean(message)} /></div><div className="field"><label htmlFor="new-password">New password</label><input id="new-password" name="password" type="password" autoComplete="new-password" minLength={12} maxLength={128} value={password} onChange={(event) => setPassword(event.target.value)} required aria-invalid={Boolean(message)} /></div>{message ? <div className="form-message form-error" role="alert">{message}</div> : null}<button className="button button-accent" type="submit" disabled={busy}>{busy ? <><LoaderCircle className="spin" size={15} /> Updating…</> : "Update password"}</button></form></section></div>;
}
