...
export async function POST(request: Request) {
  ...
  if (!parsed.success) return Response.json({ message: "Check the code and new password." }, { status: 400 });
  ...
  if (!valid) { await db.update(otps).set({ attempts: record.attempts + 1 }).where(eq(otps.id, record.id)); return Response.json({ message: "The code is invalid or has expired." }, { status: 400 }); }
  const [user] = await db.select({ id: users.id }).from(users).where(eq(users.email, parsed.data.email)).limit(1);
  if (!user) return Response.json({ message: "The code is invalid or has expired." }, { status: 400 });
  const now = new Date();
