  ...
  return <div className="simple-page page-shell">
    ...
    <div className="simple-episode-list">
      {posts.map((post) => <Link href={`/episodes/${post.slug}`} className="simple-episode" key={post.slug}>
        <span className="simple-episode-image">{post.youtubeVideoId ? <Image src={getYouTubeThumbnailUrl(post.youtubeVideoId)} alt={`Thumbnail for ${post.title}, a PIMX_ELTEX technology episode`} fill sizes="(max-width: 640px) 100vw, 300px" unoptimized /> : <span className="simple-image-fallback">PIMX_ELTEX</span>}<i><Play size={17} fill="currentColor" /></i></span>
