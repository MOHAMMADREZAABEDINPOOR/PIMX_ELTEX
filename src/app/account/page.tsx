...
export default async function AccountPage() {
  ...
  const [profileRows, activity, activeSessions] = await Promise.all([
    db.select({ name: users.name, username: users.username, email: users.email, role: users.role, declaredCountryCode: users.declaredCountryCode, emailVerifiedAt: users.emailVerifiedAt, createdAt: users.createdAt, lastLoginAt: users.lastLoginAt }).from(users).where(eq(users.id, currentUser.id)).limit(1),
  ...
  return <section className="account-page">
    ...
    <div className="account-grid">
      ...
      <aside className="account-side">
        <section className="account-panel"><header><div><span className="eyebrow">PROFILE</span><h2>Account details</h2></div></header><AccountProfileForm initialName={profile.name} initialCountry={profile.declaredCountryCode} /><dl className="account-details"><div><dt><UserRound size={14} /> Username</dt><dd>@{profile.username}</dd></div><div><dt><Mail size={14} /> Email</dt><dd>{profile.email}</dd></div><div><dt><UserRound size={14} /> Selected country</dt><dd>{countryName(profile.declaredCountryCode)}</dd></div><div><dt><Clock3 size={14} /> Member since</dt><dd>{formatDate(profile.createdAt)}</dd></div></dl></section>
        <section className="account-panel"><header><div><span className="eyebrow">SECURITY</span><h2>Active sessions</h2></div><span className="tag">{activeSessions.length}</span></header><div className="account-session-list">{activeSessions.map((session) => <article key={session.id}><strong>{session.deviceType || "Device"} · {session.browser || "Browser"}</strong><span>{session.operatingSystem || "Unknown OS"}</span><small>{locationName(session.city, session.region, session.countryCode)} · {formatDate(session.lastSeenAt)}</small></article>)}</div></section>
      </aside>
    </div>
  </section>;
}
