...
const previewPaths = Array.from(new Set(Array.from(html.matchAll(/\/api\/files\/[^"']+\/index\.html/g), (match) => match[0])));
...
const results = await Promise.all(previewPaths.map(async (path, index) => {
  ...
  const scriptsAllowed = /script-src[^;]*(?:https:|cdnjs\.cloudflare\.com)/.test(csp);
  const startAdventureReady = index !== 0 || (source.includes("Start Adventure") && source.includes("three.min.js") && scriptsAllowed);
